...
/** One provider's API-key row inside Settings. Self-contained: shows current
 *  status, lets you paste a new key or clear it, and reports the new status up. */
function KeyField({
  label,
  help,
  placeholder,
  ...
  onSave: (key: string) => Promise<api.KeyStatus>;
  onClear: () => Promise<api.KeyStatus>;
  autoFocus?: boolean;
}) {
  ...
  return (
    <div className="field" style={{ marginBottom: 18 }}>
      <label>{label}</label>
      <div style={{ color: "var(--text-dim)", fontSize: 12, marginBottom: 6 }}>{help}</div>
      <div style={{ color: "var(--text-dim)", fontSize: 13, marginBottom: 6 }}>
        {status.set ? <>Set · ending in <code>{status.preview}</code></> : <em>Not set</em>}
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        ...
        />
        <button className="primary" onClick={doSave} disabled={busy || !key.trim()}>
          {busy ? "…" : savedTick ? "Saved" : "Save"}
        </button>
        {status.set && (
          <button onClick={doClear} disabled={busy} style={{ color: "var(--danger)" }}>Clear</button>
        )}
      </div>
...
function SettingsModal({
  ...
  initialStatus: api.KeyStatus;
  onClose: () => void;
  onSaved: (s: api.KeyStatus) => void;
}) {
  const [anthropic, setAnthropic] = useState<api.KeyStatus>(initialStatus);
  const [openai, setOpenai] = useState<api.KeyStatus>({ set: false, preview: null });
  const [gemini, setGemini] = useState<api.KeyStatus>({ set: false, preview: null });
  // Global defaults applied to every profile.
  const [defBrief, setDefBrief] = useState("");
  const [defVoice, setDefVoice] = useState("");
  const [defBusy, setDefBusy] = useState(false);
  const [defSaved, setDefSaved] = useState(false);

  // Load the OpenAI + Gemini key statuses and global defaults when the modal opens.
  useEffect(() => {
    api.getOpenAIKeyStatus().then(setOpenai).catch(() => { /* leave as not-set */ });
    api.getGeminiKeyStatus().then(setGemini).catch(() => { /* leave as not-set */ });
    api.getDefaults().then((d) => { setDefBrief(d.defaultInterviewBrief); setDefVoice(d.defaultVoiceSample); }).catch(() => {});
  }, []);

  const saveDefaults = async () => {
    setDefBusy(true);
    try {
      await api.setDefaults({ defaultInterviewBrief: defBrief, defaultVoiceSample: defVoice });
      setDefSaved(true);
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <p className="modal-sub">
            API keys are stored locally on this machine and only used to call the model on your
            behalf. Set a profile's model (Claude, OpenAI, or Gemini) to choose which one answers.
          </p>

          <KeyField
            label="Anthropic API key"
            help={<>For Claude models. Get one at console.anthropic.com → API Keys.</>}
            placeholder="sk-ant-..."
            status={anthropic}
            autoFocus
            onSave={async (k) => { const s = await api.setAnthropicKey(k); setAnthropic(s); onSaved(s); return s; }}
            onClear={async () => { const s = await api.setAnthropicKey(null); setAnthropic(s); onSaved(s); return s; }}
          />

          <KeyField
            label="OpenAI API key"
            help={<>For GPT-5.5. Get one at platform.openai.com → API keys.</>}
            placeholder="sk-..."
            status={openai}
            onSave={async (k) => { const s = await api.setOpenAIKey(k); setOpenai(s); return s; }}
            onClear={async () => { const s = await api.setOpenAIKey(null); setOpenai(s); return s; }}
          />

          <KeyField
            label="Google Gemini API key"
            help={<>For Gemini 3.1 Pro / 3.5 Flash. Get one at aistudio.google.com → API keys.</>}
            placeholder="AIza..."
            status={gemini}
            onSave={async (k) => { const s = await api.setGeminiKey(k); setGemini(s); return s; }}
            onClear={async () => { const s = await api.setGeminiKey(null); setGemini(s); return s; }}
          />

          <hr style={{ border: "none", borderTop: "1px solid var(--border)", margin: "20px 0 16px" }} />

          <h3 style={{ margin: "0 0 4px", fontSize: 15 }}>Default profile info</h3>
          <p className="modal-sub" style={{ marginTop: 0 }}>
            Applied to <strong>every</strong> profile automatically — set your background, voice/tone,
            and constraints once here. A profile's own brief (if you add one) is appended after this.
          </p>

          <div className="field" style={{ marginBottom: 14 }}>
            <label>Default interview brief (all profiles)</label>
            <div style={{ color: "var(--text-dim)", fontSize: 12, marginBottom: 6 }}>
              Background, remote requirement, culture-fit, how you talk about your work — anything true for you in every interview.
            </div>
            <textarea
              value={defBrief}
              onChange={(e) => setDefBrief(e.target.value)}
              rows={8}
              placeholder="e.g. Remote only. Senior full-stack / AI engineer. Background: …  Hobbies: …"
          ...
          <div className="field" style={{ marginBottom: 8 }}>
            <label>Default voice sample (tone Brandon mirrors)</label>
            <div style={{ color: "var(--text-dim)", fontSize: 12, marginBottom: 6 }}>
              A paragraph in your own words. Brandon mirrors this rhythm/vocabulary in answers.
            </div>
            <textarea
              value={defVoice}
              onChange={(e) => setDefVoice(e.target.value)}
              rows={4}
              placeholder="Write a few sentences exactly how you'd actually talk…"
              style={{ width: "100%", resize: "vertical", fontFamily: "inherit" }}
              spellCheck={false}
            />
          </div>
          <button className="primary" onClick={saveDefaults} disabled={defBusy}>
            {defBusy ? "Saving…" : defSaved ? "Saved ✓" : "Save defaults"}
          </button>
        </div>
        <div className="modal-actions">
...
• Avoid mentioning: <anything you don't want Claude to bring up>`}
      ...
      <div>
        ...
        <select value={model} onChange={(e) => setModel(e.target.value as Profile["model"])} style={{ width: 280 }}>
          <optgroup label="Anthropic (needs sk-ant- key)">
            <option value="claude-opus-4-8">Claude Opus 4.8</option>
            <option value="claude-sonnet-4-6">Claude Sonnet 4.6</option>
            <option value="claude-opus-4-7">Claude Opus 4.7</option>
            <option value="claude-haiku-4-5-20251001">Claude Haiku 4.5</option>
          </optgroup>
          <optgroup label="OpenAI (needs sk- key)">
            <option value="gpt-5.5">GPT-5.5</option>
          </optgroup>
          <optgroup label="Google (needs AIza key)">
            <option value="gemini-3.1-pro-preview">Gemini 3.1 Pro</option>
            <option value="gemini-3.5-flash">Gemini 3.5 Flash</option>
          </optgroup>
        </select>
        <div className="hint" style={{ fontSize: 12, color: "var(--text-dim)", marginTop: 4 }}>
          Switch a profile's model to compare Claude vs OpenAI on the same questions.
        </div>
      </div>
